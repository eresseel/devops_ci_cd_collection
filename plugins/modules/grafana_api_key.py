...
class GrafanaAPIKeysManager:
    def __init__(self, api_url, grafana_user, grafana_password, org_name):
...
def main():
    module_args = {
        ...
        "grafana_user": {"type": "str", "required": True},
        "grafana_password": {"type": "str", "required": True, "no_log": True},
    ...
    api_keys_manager = GrafanaAPIKeysManager(url, grafana_user, grafana_password, org_name)
    ...
    try:
        ...
        if state == "present":
            if not api_keys or name not in [api_key["name"] for api_key in api_keys]:
                api_keys_manager.add_api_key_to_org_folder(name, role, seconds_to_live)
                module.exit_json(changed=True, msg="API keys retrieved and added")
            else:
